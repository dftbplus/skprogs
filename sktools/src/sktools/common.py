...
def writefloats(fp, nums, indent=0, indentstr=None, numperline=4,
    ...
    for ii in range(nline):
        fp.write(lineform.format(
            *nums1d[ii * numperline:(ii + 1) * numperline]))
...
def split_fortran_fields(txt, maxsplit=0):
    """"Splits a line containing Fortran (numeric) fields."""
    return [ field
             for field in PAT_FORTRAN_SEPARATOR.split(txt, maxsplit=maxsplit)
             if len(field) > 1 ]
...
def get_shellvalues_list(node, query, converter):
    values = []
    for ll, shellname in enumerate(ANGMOM_TO_SHELL):
...
def hsd_node_factory(classtype, classes, node, query):
    ...
    if myclass is None:
        raise hsd.HSDInvalidTagException("Unknown {} '{}'".format(classtype,
                                                                  node.tag))
